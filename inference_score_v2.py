...
from diffusers import EulerAncestralDiscreteScheduler, AutoencoderKL, StableDiffusionXLAdapterPipeline
...
# --- Pipeline 初始化 ---
def initialize_controlnet_pipeline(adapter_config_name, DEVICE):
...
def generation_phase(pipe, dataset, TEMP_GEN_DIR, TEMP_REAL_DIR, TEMP_PROMPTS_FILE, TEMP_LPIPS_FILE, IMAGE_SIZE,
                     ...
                     CONTROLNET_TYPE, DEVICE, lpips_model, pose_processor=None):
    ...
    for i, example in enumerate(data_iterator):
        ...
        # 2. 结构 LPIPS 计算
        extracted_cond_img = re_extract_condition_map(generated_image, CONTROLNET_TYPE, pose_processor).resize(
            IMAGE_SIZE)
...
# =======================================================
def metric_phase(TEMP_GEN_DIR, TEMP_REAL_DIR, TEMP_PROMPTS_FILE, TEMP_LPIPS_FILE, DEVICE,
                 avg_structural_lpips_from_phase1=None):
    ...
    if final_lpips_score is None:
        if os.path.exists(TEMP_LPIPS_FILE):
            ...
            raise FileNotFoundError(
                f"LPIPS file not found at {TEMP_LPIPS_FILE}. Please run generation phase or pass LPIPS manually.")
