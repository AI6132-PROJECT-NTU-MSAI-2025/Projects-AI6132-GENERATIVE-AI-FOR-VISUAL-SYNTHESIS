...
# --- Pipeline 初始化 ---
def initialize_controlnet_pipeline(base_model_path, controlnet_path, DEVICE):
    """初始化 ControlNet Pipeline"""
    controlnet = ControlNetModel.from_pretrained(controlnet_path, torch_dtype=torch.float16)
    print("--- 正在加载 ControlNet 评估 Pipeline ---")
    ...
    pipe_with_controlnet.to(DEVICE)
    pipe_with_controlnet.enable_xformers_memory_efficient_attention()
    return pipe_with_controlnet

# =======================================================
# 阶段一：图像生成和 LPIPS 计算 (耗时操作)
# =======================================================
def generation_phase(pipe, dataset, TEMP_GEN_DIR, TEMP_REAL_DIR, TEMP_PROMPTS_FILE, TEMP_LPIPS_FILE, IMAGE_SIZE, 
                     caption_col, conditioning_image_col, real_image_col, 
                     control_strength, num_inference_steps, NUM_SAMPLES_TO_EVALUATE, 
                     CONTROLNET_TYPE, DEVICE, lpips_model, pose_processor):
    ...
    for i, example in enumerate(data_iterator):
        ...
        # 2. 结构 LPIPS 计算 注意！此结果废弃，这个代码只能识别人，不识别狗
        extracted_cond_img = re_extract_condition_map(generated_image, CONTROLNET_TYPE, pose_processor).resize(IMAGE_SIZE)
        tensor_extracted = pil_to_lpips_tensor(extracted_cond_img, DEVICE)
        tensor_input_cond = pil_to_lpips_tensor(control_img, DEVICE)

        with torch.no_grad():
            lpips_score = lpips_model(tensor_extracted, tensor_input_cond).item()
...
def metric_phase(TEMP_GEN_DIR, TEMP_REAL_DIR, TEMP_PROMPTS_FILE, TEMP_LPIPS_FILE, DEVICE, avg_structural_lpips_from_phase1=None):
    ...
    if len(generated_files) != num_samples:
        print(f" 警告：实际找到的图片数量 ({len(generated_files)}) 与 Prompt 数量 ({num_samples}) 不一致。")
...
if __name__ == "__main__":
    ####这个脚本的LPIPS计算是尝试比较骨骼图，但openpose无法输出狗狗的骨骼，不要使用，请使用inference_LPIPS的结果
    ...
    TEMP_GEN_DIR = "temp_controlnet_generated_images"
    ...
    controlnet_path = "./check_points/checkpoint-24000"
    control_strength = 0.7
    num_inference_steps = 20
    OUTPUT_FILE_NAME = "controlnet_evaluation_results.txt"
    ...
    if RUN_GENERATION_PHASE:
        pipe = initialize_controlnet_pipeline(base_model_path, controlnet_path, DEVICE)
    ...
    if RUN_GENERATION_PHASE:
        ...
        avg_structural_lpips_result = generation_phase(
            pipe, dataset_dict, TEMP_GEN_DIR, TEMP_REAL_DIR, TEMP_PROMPTS_FILE, TEMP_LPIPS_FILE, IMAGE_SIZE, 
            caption_col, conditioning_image_col, real_image_col, 
            control_strength, num_inference_steps, NUM_SAMPLES_TO_EVALUATE, 
            CONTROLNET_TYPE, DEVICE, lpips_model, pose_processor
        )
    ...
    with open(OUTPUT_FILE_NAME, 'w') as f:
        f.write("--- ControlNet 评估结果 ---\n")
        f.write(f"Timestamp: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"ControlNet Path: {controlnet_path}\n")
        f.write(f"ControlNet Type: {CONTROLNET_TYPE}\n")
