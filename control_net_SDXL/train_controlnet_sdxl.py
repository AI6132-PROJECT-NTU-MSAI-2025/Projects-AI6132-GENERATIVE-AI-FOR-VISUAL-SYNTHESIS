...
def get_train_dataset(args, accelerator):
    ...
    with accelerator.main_process_first():
        ...
        train_dataset = train_dataset.cast_column(image_column, daIMG())
        train_dataset = train_dataset.cast_column(conditioning_image_column, daIMG())
...
def main(args):
    ...
    def save_training_history(output_dir, global_step, losses):
        ...
        if not os.path.exists(save_path):
            # 这通常会在 checkpointing_steps 的逻辑中创建，这里作为保险
        ...
        output_filename = os.path.join(save_path, 'training_history.pkl')

        with open(output_filename, 'wb') as f:
            pickle.dump(history_data, f)

        logger.info(f"(train_losses) saved to {output_filename}")
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            # --- [新增] 恢复历史损失逻辑 (重要) ---
            if accelerator.is_main_process:
                # 检查检查点文件夹内是否存在 loss 文件
                history_file = os.path.join(args.output_dir, path, 'training_history.pkl')
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ''' #if you want to save disk space, enable the following codes
                        ...
                        controlnet_to_save.save_pretrained(os.path.join(save_path, "controlnet"))
                        logger.info(f"Checkpoint ({global_step})：saved ControlNet weight (only safetensor)")
                        is_final_step = (global_step == args.max_train_steps)

                        if is_final_step:
                            accelerator.save_state(save_path)
                            logger.info(f"final Checkpoint ({global_step})：saved checkpoint")
                        '''

                        # normal check_point_save
                        save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
                        accelerator.save_state(save_path)
                        ...
                        if accelerator.is_main_process:
                            # 仅在主进程上保存loss
                            save_training_history(args.output_dir, global_step, train_losses)
